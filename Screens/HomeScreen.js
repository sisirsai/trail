...
export default class HomeScreen extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    var class_ref = await db.ref('class-8th/').on('value', (data) => {
      console.log(all_students);
      var all_students = [];
  ...
  updateAttendence(roll_no, status) {
    ...
    today = dd + '-' + mm + '-' + yyyy;
    var ref_path = id;
    var class_ref = db.ref('class-8th/' + id);
    class_ref.update({
      [today]: status,
    });
  }
                  ...
                  <TouchableOpacity
                    style={
                      this.state.absentPressedList.includes(index)
                        ? [styles.absentButton, { backgroundColor: 'green' }]
                        : styles.absentButton
                    }
                    onPress={() => {
                  ...
                  <TouchableOpacity
                    style={
                      this.state.presentPressedList.includes(index)
                        ? [styles.presentButton, { backgroundColor: 'red' }]
